#!/usr/bin/env node
/**
 * Fetch Dish Images for Vienna Restaurants
 * Scrapes images from Uber Eats, Wolt, Just Eat and updates dishes in Firestore
 *
 * Usage:
 *   node fetch-vienna-dish-images.cjs                    # Dry run - show what would be fetched
 *   node fetch-vienna-dish-images.cjs --execute         # Actually fetch and update images
 *   node fetch-vienna-dish-images.cjs --venue=<id>      # Process specific venue only
 */
...
async function fetchViennaDishImages() {
  console.log('\n=== FETCH VIENNA DISH IMAGES ===\n');
  ...
  // Filter to Vienna area
  let viennaVenues = allVenues.filter(v => {
  ...
  if (SPECIFIC_VENUE) {
    viennaVenues = viennaVenues.filter(v => v.id === SPECIFIC_VENUE);
  }
