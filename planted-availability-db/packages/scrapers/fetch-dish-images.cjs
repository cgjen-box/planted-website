#!/usr/bin/env node
/**
 * Fetch Dish Images from Delivery Platforms
 * Scrapes images from Uber Eats, Just Eat, etc. and updates dishes in Firestore
 *
 * Usage:
 *   node fetch-dish-images.cjs                    # Dry run - show what would be fetched
 *   node fetch-dish-images.cjs --execute         # Actually fetch and update images
 *   node fetch-dish-images.cjs --venue=<id>      # Process specific venue only
 */
...
async function fetchDishImages() {
  ...
  // Filter to Zurich area
  let zurichVenues = allVenues.filter(v => {
    const lat = v.location?.latitude || v.location?._latitude || 0;
    const lng = v.location?.longitude || v.location?._longitude || 0;
    return lat > 47.3 && lat < 47.5 && lng > 8.4 && lng < 8.7;
  });

  if (SPECIFIC_VENUE) {
    zurichVenues = zurichVenues.filter(v => v.id === SPECIFIC_VENUE);
  }
  ...
  for (const venue of zurichVenues) {
