...
async function analyzeBerlinDishImages() {
  console.log('\n=== BERLIN DISH IMAGES ANALYSIS ===\n');
  ...
  // Filter to Berlin area (lat ~52.52, lng ~13.40)
  const berlinVenues = allVenues.filter(v => {
    ...
    // Within ~20km of Berlin center
    return lat > 52.3 && lat < 52.7 && lng > 13.2 && lng < 13.6;
  });

  console.log(`Found ${berlinVenues.length} Berlin-area venues\n`);
