...
async function analyzeZurichDishImages() {
  console.log('\n=== ZURICH DISH IMAGES ANALYSIS ===\n');

  // Get Zurich venues (within ~10km of city center)
  ...
  // Filter to Zurich area (lat ~47.37, lng ~8.54)
  const zurichVenues = allVenues.filter(v => {
    const lat = v.location?.latitude || v.location?._latitude || 0;
    const lng = v.location?.longitude || v.location?._longitude || 0;
    // Within ~15km of Zurich center
    return lat > 47.3 && lat < 47.5 && lng > 8.4 && lng < 8.7;
  });

  console.log(`Found ${zurichVenues.length} Zurich-area venues\n`);
